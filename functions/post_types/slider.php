...
function slider_score_meta_box( $object, $box ) {
	...
	?><div class="ci-cf-wrap"><?php
		ci_metabox_open_tab( '' );
			ci_metabox_guide( __( 'If someone clicks on a slider item, the Slide URL is the link that they will be visiting. If you leave it empty, linking for this slide will be disabled.', 'ci_theme' ) );
			ci_metabox_input( 'ci_cpt_slider_url', __( 'Slide URL:', 'ci_theme' ), array( 'esc_func' => 'esc_url' ) );

			ci_metabox_guide( sprintf( __( 'Tthe video must be hosted in one of the <a href="%s">WordPress supported providers</a>.', 'ci_theme' ), 'http://codex.wordpress.org/Embeds' ) );
